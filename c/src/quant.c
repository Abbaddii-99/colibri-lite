...
void matmul(float *y, const float *x, const float *W, int S, int I, int O){
    ...
    for (int o=0;o<O;o++){ const float *w=W+(int64_t)o*I;
        for (int s=0;s<S;s++){ const float *xs=x+(int64_t)s*I; float a=0; int i=0;
            ...
            for(;i+8<=I;i+=8){ ac0=vfmaq_f32(ac0, vld1q_f32(xs+i),   vld1q_f32(w+i));
                               ac1=vfmaq_f32(ac1, vld1q_f32(xs+i+4), vld1q_f32(w+i+4)); }
            a=vaddvq_f32(vaddq_f32(ac0,ac1));
#endif
            for(;i<I;i++) a+=xs[i]*w[i]; y[(int64_t)s*O+o]=a; } }
}
...
void matmul_q_idot(float *y, const int8_t *xq, const float *sx, const int8_t *q,
...
void matmul_i4_idot(float *y, const int8_t *xq, const float *sx, const uint8_t *q4,
                   const float *scale, int S, int I, int O){
    int rb=(I+1)/2;
    #pragma omp parallel for schedule(static)
    for(int o=0;o<O;o++){ const uint8_t *w=q4+(int64_t)o*rb; float sc=scale[o];
...
void qt_alloc(QT *t, int O, int I, int bits){
    t->fmt = bits==32 ? 0 : bits==8 ? 1 : bits==16 ? 0 : bits==4 ? 2 : 3;
    t->O=O; t->I=I; t->qf=NULL; t->q8=NULL; t->q4=NULL; t->s=NULL; t->cuda_eligible=0;
    int64_t n=(int64_t)O*I;
    if(t->fmt==0){ t->qf=calloc(n,sizeof(float)); if(!t->qf){ fprintf(stderr,"OOM qt_alloc\n");exit(1);} }
    else if(t->fmt==1){ t->q8=calloc(n,1); t->s=calloc(O,sizeof(float)); if(!t->q8||!t->s){ fprintf(stderr,"OOM qt_alloc\n");exit(1);} }
    else if(t->fmt==3){ t->q4=calloc((size_t)O*((I+3)/4),1); t->s=calloc(O,sizeof(float)); if(!t->q4||!t->s){ fprintf(stderr,"OOM qt_alloc\n");exit(1);} }
    else{ t->q4=calloc((size_t)O*((I+1)/2),1); t->s=calloc(O,sizeof(float)); if(!t->q4||!t->s){ fprintf(stderr,"OOM qt_alloc\n");exit(1);} }
}

void qt_fill(QT *t, const float *w, int bits){
    if(bits==32 || bits==16) memcpy(t->qf, w, (size_t)t->O*t->I*4);
    else if(bits==8) quantize_rows(w,t->q8,t->s,t->O,t->I,bits);
    else if(bits==4) pack_int4(w,t->q4,t->s,t->O,t->I,bits);
    else pack_int2(w,t->q4,t->s,t->O,t->I,bits);
}

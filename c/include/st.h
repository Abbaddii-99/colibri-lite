...
static void st_init(shards *S, const char *snap_dir) {
    ...
    for (int fi = 0; fi < nf; fi++) {
        ...
        for (int i = 0; i < root->len; i++) {
            ...
            if (S->n == S->cap) { S->cap *= 2; S->t = realloc(S->t, S->cap*sizeof(st_tensor)); if(!S->t){perror("realloc");exit(1);} }
            st_tensor *t = &S->t[S->n++];
    ...
    if(!S->hidx){perror("malloc hidx");exit(1);}
...
/* libera la memoria e chiude i file descriptors */
static void st_free(shards *S) {
    for (int i = 0; i < S->n; i++) free(S->t[i].name);
    for (int i = 0; i < S->nfd; i++) { close(S->fds[i]); free(S->paths[i]); if (S->dfds[i] >= 0) close(S->dfds[i]); }
    free(S->t); free(S->hidx); memset(S, 0, sizeof(*S));
}
